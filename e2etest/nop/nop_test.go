	...
	for _, testCase := range TestCases {
		t.Run(testCase.Name, func(t *testing.T) {
			params := driver.NewTestParams(t, map[string]string{
				"WasmFile": filepath.Join(env.GetBazelBinOrDie(), "extensions/nop/nop.wasm"),
			}, test.ExtensionE2ETests)
			if testCase.Host != "" {
				params.Vars["Host"] = testCase.Host
			}
			params.Vars["ServerHTTPFilters"] = params.LoadTestData("e2etest/nop/testdata/server_filter.yaml.tmpl")
